...
int main() {
    ...
    for (int i = 0; i < numRules; i++) {
        ...
        if (sscanf(input, " %c -> %[^\n]", &rules[i].nonTerminal, rules[i].production) != 2) {
    ...
    for (int i = 0; i < numRules; i++) {
        if (!containsSymbol(processedNonTerminals, rules[i].nonTerminal)) {
            ...
            fprintf(fp, "First(%c): { ", rules[i].nonTerminal);
            for (int j = 0; j < firstSet.count; j++) {
                fprintf(fp, "%c ", firstSet.symbols[j]);
            }
            fprintf(fp, "}\n");
            ...
            fprintf(ft, "Follow(%c): { ", rules[i].nonTerminal);
            for (int j = 0; j < followSet.count; j++) {
                fprintf(ft, "%c ", followSet.symbols[j]);
            }
            fprintf(ft, "}\n");
